...
`scene_lock_misses` is expected to be `0` on the two **gated** benchmark sessions (`steady_state_render`, `high_mutation`) because those scenarios are single-threaded — they `.lock().await` the scene directly with no concurrent gRPC/MCP scene-mutation handlers contending for the lock, so the genuine measured value is `0`. The gate's `REQUIRED_SESSIONS` is exactly those two sessions; the committed-zero baseline locks the no-contention invariant for the production-shaped paths.
...
**Healthy vs. concerning ranges.** For the two *gated* sessions, anything above `0` is concerning (a real regression: a handler is holding the scene lock long enough to starve the frame loop). For the `scene_lock_contention` reference session, a *high* miss count is the expected, healthy outcome (it proves the contention path is live and the counter wiring works end-to-end); a *zero* there would be the concerning signal (it would mean the contention scenario stopped actually contending and the gate's protection is unverified).

**No CI threshold change was made for this work.** A non-zero baseline ceiling is intentionally *not* set yet: doing so would require either gating the contention session (which is a synthetic worst case, not a production budget) or moving `scene_lock_misses` into `BASELINE_COUNTERS` for the production sessions (which must stay at zero). If we later want to gate a *bounded* contention budget for the production sessions under a realistic concurrent load, that is a follow-up: add the contended session to a gated set (or a new budget table) and set its documented ceiling in `BASELINE_COUNTERS` from observed data — not by guessing.

True cross-run *history* (storing prior-run numeric values and reporting deltas) is implemented as a separate **informational** trend surface (hud-kkx15): `scripts/ci/report_telemetry_trend.py` consumes the gate's `budget-gate.json` output, and the `windows-performance-budget` job persists each run's summary in a branch-scoped `actions/cache` entry (keyed per-run, restored via `restore-keys` prefix). On each run it restores the newest prior summary and appends a numeric-delta table (e.g. `frame_time p99 14.1ms -> 14.8ms, +5%, within budget`) to the CI step summary. This is deliberately non-failing — it surfaces within-budget drift the hard gate cannot see; the committed-zero/locked baselines remain the PASS/FAIL authority. The first-run / no-prior-history case is handled gracefully (notice printed, no error).
