...
| `scene_lock_misses` on `scene_lock_paced_contention` | `1..=20` per 180-frame CI run | none | Production-shaped paced contention counter gate |
...
|------|--------------|-------|---------------|--------|
| ~~`crates/tze_hud_runtime/src/windowed.rs`~~ | 10,061 lines (2026-06); removed as a file by 2026-06-20 | window lifecycle -> `windowed/lifecycle.rs`; input routing -> `windowed/input_dispatch.rs` + `windowed/keyboard.rs`; overlay hittest -> `windowed/hittest.rs`; widgets/local-feedback -> `windowed/widgets.rs`; MCP/gRPC startup -> `windowed/network.rs`; portal and test seams -> `windowed/portal.rs` + `windowed/test_support.rs` | `hud-4icehz` (decompose into `windowed/`) | **Closed** - closing PR tranche: #920, #922, #930, #931, #934; current largest split file is `windowed/portal.rs` at 2,849 lines |
