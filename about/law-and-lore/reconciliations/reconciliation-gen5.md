...
|------|--------|----------|
| **Widget system** (5 delta specs) | PARTIAL | Widget ontology, parameter schema, SVG rasterization, publishing, contention all implemented. **2 P1 gaps remain**: ClearWidgetMutation unwired (GAP-1), Widget TTL expiry not enforced (GAP-2). 2 P2 gaps: type ID validation missing, occupancy per-policy resolution partial. |
...
| PARTIAL | Widget system | ~5% | 2 P1 gaps block widget epic closure |
...
### GAP-2: Widget TTL expiry not enforced (P1)
- **Spec**: Widget publications with TTL must auto-expire
- **Code**: `WidgetPublishRecord.expires_at_wall_us` is populated but never checked; no `drain_expired_widget_publications()` in frame loop
- **Impact**: Widget publications persist indefinitely regardless of TTL
- **Bead**: hud-2c5g
...
Gen-5 is a **progress snapshot**, not a closure point. The widget system P1 gaps (GAP-1, GAP-2) must be closed before the widget-system epic can be archived.
