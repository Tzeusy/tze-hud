...
`.claude/skills/hud-projection/scripts/portal_client.py` transport. It supplies
fixed fixture IDs, timestamps, content, order, and response input. The live
owner token is required for the real calls but is replaced with the canonical
...
tokenizer, vocabulary, fixture, and flow fingerprints make incompatible
...
requires explicit owner approval of every value in the candidate. Baseline
changes then require explicit owner review. Changing a tokenizer identity,
fixture fingerprint, flow fingerprint, operation set, or approval state is not a
performance comparison; the gate reports `baseline_incompatible`.
