...
**Decision:** The Rust harness writes canonical machine-readable JSON artifacts into the Layer 4 benchmark directory and may also append a stable summary row to the historical CSV ledger consumed by `/user-test`.
...
- Artifact-only output with no summary row: rejected because it would break the existing `/user-test` audit workflow.
...
**Decision:** The harness uses the same target-registry and benchmark-identity concepts already established in `.claude/skills/user-test`.
...
- **[Wrapper integration churn]** The current `/user-test` skill already records results and comparisons. → Mitigation: preserve its CSV schema and target registry, and switch only the gRPC widget path to the Rust binary first.
...
4. Add CSV summary compatibility and switch `/user-test` gRPC benchmarking to the Rust binary.
...
- Should the historical CSV be owned under a first-party benchmark-ledger path (for example under `test_results/`), or remain primarily managed via `/user-test` wrappers?
