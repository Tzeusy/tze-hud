...
- [ ] 1.2 Create `widget.toml` manifest declaring `name = "progress-bar"`, `version = "1.0.0"`, `description = "Horizontal progress bar with configurable fill and label"`, `default_geometry = { x = 0, y = 0, width = 300, height = 20 }`, `default_contention_policy = "LatestWins"`, parameter_schema with three parameters (progress: f32 min=0.0 max=1.0 default=0.0, label: string max_length=32 default="", fill_color: color default=[74,158,255,255]), layers array referencing track.svg (no bindings) and fill.svg (three bindings: linear progress->fill-bar.width attr_min=0 attr_max=296, direct fill_color->fill-bar.fill, text-content label->label-text.text-content)
- [ ] 1.3 Create `track.svg` with `viewBox="0 0 300 20"` containing a single `<rect id="track-bg" x="0" y="0" width="300" height="20" rx="10" ry="10" fill="{{color.backdrop.default}}" fill-opacity="0.3"/>` — background track with rounded end-caps, token-driven color at 30% opacity
- [ ] 1.4 Create `fill.svg` with `viewBox="0 0 300 20"` containing: (a) `<rect id="fill-bar" x="2" y="2" width="0" height="16" rx="8" ry="8" fill="{{color.text.accent}}"/>` — dynamic fill bar with token-driven default color, 2px inset from track edges, (b) `<text id="label-text" x="150" y="10" text-anchor="middle" dominant-baseline="central" font-size="11" fill="{{color.text.primary}}"></text>` — centered label text with token-driven primary color
