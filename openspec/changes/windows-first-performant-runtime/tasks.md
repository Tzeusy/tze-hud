...
- [x] 1.7 Spawn parent epic + first calibration bead under `hud-9wljr`
- [x] 1.8 Commit and push (mandatory landing protocol)

## 2. Performance baseline (next)

- [x] 2.1 Identify or stand up a reference Windows machine; record CPU, GPU, RAM, display, OS build in `docs/reports/`
- [x] 2.2 Run existing benches (`examples/widget_publish_load_harness`, scene-graph mutation throughput, frame-time histograms, overlay composite cost) on the reference machine
- [x] 2.3 Write a baseline report under `docs/reports/windows_perf_baseline_2026-05.md` covering: frame time p50/p99/p99.9, input latency triple, widget raster cost, overlay composite cost, idle CPU/GPU/memory, multi-agent soak behavior
- [x] 2.4 Compare baseline to the proposed targets in `design.md` §1; identify the top three gaps and their suspected causes

## 3. Lock budgets

- [x] 3.1 Update `about/craft-and-care/engineering-bar.md` §2 with the calibrated Windows-only performance budgets and reference-hardware tag
- [x] 3.2 Add CI gates in the existing benchmark harness that fail PRs which regress past the locked budgets
- [x] 3.3 Document the reference-hardware procurement / claim path
...
- [x] 6.1 Resolve `hud-ggntn.7` (gen-1 reconciliation) and `hud-abwjw` (validation host reachability) blockers
- [x] 6.2 Complete `hud-ggntn.10` (live Windows governance validation) and `hud-ggntn.11` (gen-2 reconcile)
- [x] 6.3 Archive cooperative-hud-projection change after closeout report
