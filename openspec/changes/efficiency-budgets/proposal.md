...
- Add deterministic byte and token calibration for canonical `publish_to_zone`, text-stream portal turn, and `publish_to_widget` flows, with the owner-approved v1 packet (exact integers, tokenizer/vocabulary identity, fixture/flow fingerprints, and decision provenance) as the checked-in comparison authority and explicit regression thresholds.
