...
- [ ] 3.1 Implement lease request with AUTO_RENEW policy, ttl_ms 120000, capabilities [create_tiles, modify_own_tiles]
- [ ] 3.2 Verify auto-renewal fires at 75% TTL (90s) — confirm LeaseResponse with result GRANTED is received
- [ ] 3.3 Verify tile creation is rejected when no lease is active (LeaseExpired / LeaseNotFound error)

## 4. Periodic Content Updates

- [ ] 4.1 Implement 30-second content update loop: constructs ReplaceNode mutation for TextMarkdownNode with updated "Last active: Ns ago" text
- [ ] 4.2 Verify content updates produce valid MutationBatch (1 mutation, ReplaceNode only) and are accepted by the runtime
