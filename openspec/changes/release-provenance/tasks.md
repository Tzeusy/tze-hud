...
- [x] 1.1 Validate this change: `openspec validate release-provenance --strict`
- [x] 1.2 Confirm the requirement adds no runtime behavior change (release/provenance only)
- [x] 1.3 Confirm signing is explicitly optional/deferred for v1

## 2. Implementation

- [x] 2.1 CI: produce `tze_hud.exe` (cross `x86_64-pc-windows-gnu`) and an auto-generated `tze_hud.exe.sha256` published as a workflow artifact
- [x] 2.2 Update README §1.2 to point at the pipeline-generated checksum instead of the manual `sha256sum` instruction (README.md:225)
- [x] 2.3 Document the verification procedure (compute + compare before activation) in the deployment docs / deployment automation
