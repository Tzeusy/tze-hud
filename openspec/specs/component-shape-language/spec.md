...
**Runtime registration status (not yet wired):** as of this archive the runtime loader recognizes only the six v1 variants — `text-portal` is absent from `ComponentType::ALL` / `ComponentType::from_name` (`crates/tze_hud_config/src/component_types.rs`), so a conforming `component_type = "text-portal"` profile currently fails to load with `PROFILE_UNKNOWN_COMPONENT_TYPE` (`crates/tze_hud_config/src/component_profiles.rs`), and the per-part `OpaqueBackdrop` readability enforcement below has no runtime validation path. Registering `text-portal` in the loader and wiring its readability enforcement is the promotion's runtime step, tracked as a separate follow-up (reconciliation finding F4, `docs/reports/promotion-epic-reconciliation-20260711.md`); it is out of scope for this styling-contract sync. Where this requirement says `text-portal` is "recognized" after the RFC 0013 §7.2 gate, that recognition is realized only once F4 lands.
Scope: post-v1
