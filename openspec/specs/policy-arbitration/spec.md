...
Defines the runtime's seven-level arbitration stack that governs every mutation
admitted to the scene. Covers cross-level and within-level conflict resolution,
human override semantics (Level 0), safety response (Level 1), privacy
evaluation and redaction ownership (Level 2), security and capability
enforcement (Level 3), attention management and quiet hours (Level 4), resource
budget enforcement and degradation (Level 5), and zone contention resolution
(Level 6). Also covers per-frame, per-event, and per-mutation evaluation
pipelines; ArbitrationOutcome types; freeze semantics; capability registry
...
Dynamic policy rules that can be loaded or modified at runtime are deferred to post-v1. V1 implementations MUST treat all policy rules as static and MUST derive them exclusively from configuration at load time. Runtime modification of policy rules MUST NOT be supported in v1.
Source: RFC 0009 §16
Scope: post-v1

#### Scenario: Deferred dynamic policy
- **WHEN** v1 is deployed
- **THEN** all policy rules are static and defined at configuration load time; no runtime modification path exists
