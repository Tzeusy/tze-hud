...
Defines the compositor's timing architecture: the four clock domains (display/vsync, monotonic, network/wall, media), how agent-supplied timestamps are validated and scheduled, how sync groups coordinate atomic multi-tile updates, and how clock drift is detected and corrected. This is a FOUNDATION capability — all other specs that reference `present_at_wall_us`, `expires_at_wall_us`, sync groups, or clock domains depend on the contracts defined here.
...
Media clock integration (GStreamer pipeline clock, AV synchronization, PTS-to-frame mapping, word-highlighting/subtitle timing, sync groups with media_clock_binding) MUST NOT be implemented in v1. V1 ships sync groups for scene-level coordination only; media clock integration is deferred to post-v1.
Source: RFC 0003 §6
Scope: post-v1
