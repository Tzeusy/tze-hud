...
#### Scenario: question publish piggybacks one bounded viewer turn
- **WHEN** an attached LLM session calls `publish_output` with `expects_reply = true` while HUD input is eligible
- **THEN** the publish response SHALL include at most one FIFO `pending_input.items` entry and its `remaining_count`
- **AND** that entry SHALL have the same delivery state transition, delivered timestamp, repaint scheduling, audit behavior, and later acknowledgement semantics as an explicit `get_pending_input` result

#### Scenario: question publish has no eligible viewer turn
- **WHEN** an attached LLM session calls `publish_output` with `expects_reply = true` and no eligible HUD input exists
- **THEN** the output SHALL still be accepted and the compact response SHALL omit `pending_input`
- **AND** no inbox item SHALL transition to `delivered`

#### Scenario: piggyback preserves terminal acknowledgement and output replay
- **WHEN** a piggybacked input is terminally acknowledged and the same logical question output is replayed
- **THEN** repeating the same terminal acknowledgement SHALL remain idempotent
- **AND** the replay SHALL NOT redeliver the terminal input or create a second delivery transition
...
- **THEN** the serving adapter SHALL wait up to `wait_ms` milliseconds (capped at the configured maximum) for pending input to become available, polling the authority at a sub-second interval
- **AND** the call SHALL return as soon as one or more pending items are found, or return an accepted response with an empty item list after the wait elapses

#### Scenario: long-poll is served adapter-side and is side-effect-free
- **WHEN** a `get_pending_input` call with `wait_ms` elapses with no items available
- **THEN** the adapter SHALL NOT have transitioned any inbox item to `delivered` state during the wait
- **AND** the runtime drain loop and windowed event thread SHALL NOT have been blocked by the poll wait
...
#### Scenario: owner token expires after its configured TTL
- **WHEN** a non-attach owner operation presents an owner token whose attach-time deadline (`server_timestamp_wall_us + owner_token_ttl_wall_us`, default 24 hours) has passed
