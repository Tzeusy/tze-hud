...
if [ -z "${1:-}" ]; then
  echo "Usage: bash scripts/epic-report-scaffold.sh <epic-id> [repo_root]"
  echo "  epic-id: The beads epic ID (e.g., hud-abc123)"
  exit 1
fi
...
epic_json=$(normalize_issue_json "$epic_json_raw") || {
  echo "ERROR: Could not parse epic payload for $EPIC_ID"
  exit 1
}
...
files_changed=$(
  git log --all --grep="$EPIC_ID" --name-only --pretty=format: 2>/dev/null \
    | sort -u \
    | grep -v '^$' \
    | head -30 \
...
if [ "$total_children" -gt 0 ]; then
  children_table=$(
    echo "$children_json" \
      | jq -r '.[] | "| \(.id // "-") | \(.title // "-") | \(.status // "unknown") | \(.priority // "-") | \(.issue_type // .type // "task") |"'
  )
fi
...
echo "  4. Link report to epic notes"
