...
/// Default drift budget in microseconds (500 µs).
///
/// Spec: timing-model/spec.md line 198.
pub const DEFAULT_SYNC_DRIFT_BUDGET_US: u64 = 500;
...
pub struct FrameSyncDriftRecord {
    /// Worst mutation-arrival spread (µs) across all committed sync groups
    /// in this frame.
    ///
    /// `0` means no sync groups were committed this frame.
    pub sync_group_max_drift_us: u64,
...
pub struct SyncDriftHighAlert {
    ...
    /// Observed spread in microseconds.
    pub observed_drift_us: u64,
...
pub struct TileArrival {
    /// The tile.
    pub tile_id: SceneId,
    /// Wall-clock time (µs since Unix epoch) when the mutation arrived at
    /// the compositor.
    pub arrival_wall_us: u64,
}

/// Compute the drift spread for a set of committed tile arrivals.
///
/// Returns `(spread_us, slow_tiles)` where `spread_us` is the difference
...
///
/// Returns `(0, vec![])` if `arrivals` is empty or has only one entry.
pub fn compute_spread(arrivals: &[TileArrival]) -> (u64, Vec<SceneId>) {
    if arrivals.len() <= 1 {
        return (0, vec![]);
    }

    let min_arrival = arrivals.iter().map(|a| a.arrival_wall_us).min().unwrap_or(0);
    let max_arrival = arrivals.iter().map(|a| a.arrival_wall_us).max().unwrap_or(0);
    let spread = max_arrival.saturating_sub(min_arrival);
...
///   arrival records.
/// * `budget_us` — Configured drift budget in microseconds. Use
...
mod tests {
    ...
    fn arrival(tile_id: SceneId, arrival_wall_us: u64) -> TileArrival {
        TileArrival { tile_id, arrival_wall_us }
    }
    ...
    fn drift_within_budget_returns_no_alerts_and_false_flag() {
        ...
        assert_eq!(record.sync_group_max_drift_us, 300);
    ...
    fn no_groups_committed_yields_zero_drift_and_no_exceeded() {
        ...
        assert_eq!(record.sync_group_max_drift_us, 0);
        assert!(!record.sync_drift_budget_exceeded);
        assert!(record.stale_tiles.is_empty());
        assert!(alerts.is_empty());
    }
    ...
    fn multiple_groups_max_drift_is_worst_across_all() {
        ...
        assert_eq!(record.sync_group_max_drift_us, 700,
    ...
    fn default_budget_is_500_us() {
        assert_eq!(DEFAULT_SYNC_DRIFT_BUDGET_US, 500);
    }
}
