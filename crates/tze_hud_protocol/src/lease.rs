...
pub struct CachedLeaseResponse {
    /// The sequence number the server replied with originally.
...
impl LeaseCorrelationCache {
    ...
    pub fn insert(&mut self, client_sequence: u64, response: CachedLeaseResponse) {
        if self.entries.contains_key(&client_sequence) {
            // Already cached — overwrite without changing order.
            self.entries.insert(client_sequence, response);
            return;
        }
...
///   agent receives 2.
/// - All other requested priorities (2–4) are granted as-is.
///
...
mod tests {
    ...
    fn test_correlation_cache_hit_after_insert() {
        ...
        assert!(hit.granted);
        assert_eq!(hit.server_sequence, 5);
    ...
    fn test_correlation_cache_overwrite_keeps_order() {
        ...
        assert_eq!(hit.server_sequence, 11);
        assert!(!hit.granted);
