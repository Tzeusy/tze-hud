...
- `/user-test` SSH on `tzehouse-windows` (2026-06-21): the **file-user `hudbot` key auth is currently BROKEN** (host has a pending `C:\tze_hud\fix_hudbot_ssh.ps1`); the admin/console user **`tzeus` works for BOTH the SCP/file role and the process-control/admin role** with `~/.ssh/hud-ssh-key`. Drive deploys with `WIN_FILE_USER=tzeus WIN_ADMIN_USER=tzeus` until hudbot's `authorized_keys` is repaired. `tzeus` owns console session 1, so the interactive-scheduled-task capture/input pattern works as that principal (`New-ScheduledTaskPrincipal -UserId tzehouse\tzeus -LogonType Interactive`).
