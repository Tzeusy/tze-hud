...
- Scene-lock double-buffer work (`hud-ibzl4`) should not proceed from the synthetic `scene_lock_contention` data alone: `hud-iky7b` proved the counter path but produced deterministic 1.0 miss/frame saturation, while `steady_state_render` and `high_mutation` remain 0; resume only after a production-shaped contention model or live-runtime telemetry yields a data-justified ceiling (see `hud-pio04`).
