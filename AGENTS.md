...
- Operator-disabled media ingress proof must use a reachable HUD launched with `operator_disabled = true` and an authenticated local producer run with `windows_media_ingress_exemplar.py local-producer --expect-reject-code MEDIA_DISABLED`; connection refusal is launch/runtime failure, not a valid disabled-admission result.
