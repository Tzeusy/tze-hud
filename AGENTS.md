...
- Desktop-headroom audit `docs/reports/hud-48s45_desktop_headroom_assumption_audit_20260716.md` found three portability seams that exist as types/tests but not production runtime wiring: windowed cadence/budgets do not consume `RuntimeContext.profile`, `DegradationController` has no production consumer, and `CommandProcessor` has no production runtime consumer. Treat these as configuration/runtime/input reconciliation work, not as glasses/VR implementation.
