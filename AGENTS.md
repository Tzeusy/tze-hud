...
- A Chrome launch from a non-interactive Windows SSH session may return success without reaching the logged-on interactive browser; prove the loopback request before treating it as a browser rerun. An ephemeral `LogonType Interactive` scheduled task can reach the existing profile, but clean up the task/artifacts and verify the original browser/HUD/GPU state afterward. `onYouTubeIframeAPIReady` proves only that the external API script loaded; player success still requires `onReady` plus playing state or explicit operator-visible evidence.
...
- Runtime-produced input must use `tze_hud_protocol::session_server::InputEventSender`, not a raw Tokio broadcast channel: it fans transactional variants (down/up/key/focus/capture/command/IME and submit/cancel) into ordered per-session durable queues while keeping pointer move/hover, gesture, scroll-offset, and composer draft state on the bounded droppable lane.
- tzehouse exemplar/user-test gRPC runs: use a REGISTERED agent id (agent-alpha/beta/gamma per C:\tze_hud\tze_hud.toml) — any other --agent-id gets caps=[] and dies at lease acquire with PERMISSION_DENIED. Evidence runs also need `--reference-hostname "$TZE_HUD_TEST_HOST"` (the in-script REFERENCE_HOSTNAME is a scrubbed placeholder, so without the override the artifact records is_reference:false and does not count toward the promotion gate). deploy_portal_hud.sh needs `SSH_KEY="$HUD_SSH_KEY"` from tzehouse_env.sh — its default ~/.ssh/hud-ssh-key does not auth tzehouse.
- Cooperative projections (portal_projection_*) are REAPED ~60-105s after the last client op (default advisory lease TTL, no visible stale/degraded window — defect tracked hud-ccj2o): during any live session keep a `portal_client.py poll --wait-ms 30000 --rounds N` running or the portal AND its pending operator input (memory-only) vanish between your ops; recovery is idempotency-key re-attach, which auto-replays the client continuity tail (observed continuity_replayed_count working across runtime restarts and reaps).
