...
- Phase-1 portal review pattern (seen twice, 2026-06-11): perf/wiring beads got closed on PR merge while a named vector or scope item remained undelivered (hud-xq0uo's backtick flood still O(n^2) because the test had no timing assertion; hud-2ps6p's pointer-affordance leg never wired). When reviewing perf beads, empirically re-run the exact payloads named in the bead in release mode; when reviewing "wire X" beads, grep for production call-sites of the new API — component tests passing is not wiring. Adversarial/perf tests must carry timing assertions or they will silently pass on quadratic behavior.
