...
- In strict PowerShell launch scripts, do not let `schtasks /Delete` for an absent temporary task abort the exclusive Windows HUD handoff; ignore the missing-task error before stopping/restoring the benchmark HUD, or recovery can be blocked if the host drops offline mid-handoff.
...
- `diagnostic-input` must launch its OS input injector as an interactive scheduled task under `tzeus`; direct SSH PowerShell can fail `SetCursorPos`. Send the task wrapper over SSH stdin as single-line/chunked base64, because Windows PowerShell `-Command -` does not reliably continue after multiline here-strings/arrays.
...
- Windows YouTube visible-player sidecar/frame capture should not rely on direct non-interactive SSH PowerShell. Copy temp `.ps1` files, run them through an interactive scheduled task under `tzeus`, and decode returned output with UTF-16 fallback.
