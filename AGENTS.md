...
- As of gen-1 reconciliation `hud-ggntn.7`, cooperative HUD projection has `crates/tze_hud_projection` in-memory authority semantics plus skill docs/headless adapter tests, but no executable projection daemon CLI/MCP/local IPC surface or resident gRPC adapter that creates a visible text-stream portal; keep OpenSpec sync/archive blocked until those gap beads or explicit waivers land.
