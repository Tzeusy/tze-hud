...
- As of gen-1 reconciliation `hud-ggntn.7`, cooperative HUD projection had `crates/tze_hud_projection` in-memory authority semantics plus skill docs/headless adapter tests; after `hud-ggntn.8`, remaining sync/archive blockers are the resident gRPC adapter that creates a visible text-stream portal and live governance validation unless explicitly waived.
- `crates/tze_hud_projection` now ships `tze_hud_projection_authority`, a daemon-local stdio CLI surface for cooperative HUD projection operations; it retains state only for the process lifetime and emits a bounded `ProjectionResponse` plus newly written audit records for each JSON-line operation.
