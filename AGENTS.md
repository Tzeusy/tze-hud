...
- Honest resident widget-publish benchmarking is currently blocked by contract drift: RFC 0005 expects `WidgetPublishResult.request_sequence`, but `crates/tze_hud_protocol/proto/session.proto`, runtime handling, and active OpenSpec omit it; repair that seam before trusting per-request RTT numbers.
...
- On the current Windows `/user-test` target, overlay/render diagnostics can report a 2560x1440 compositor surface while scene mutation bounds still enforce 1920x1080 display space; Presence Card placement should use `--tab-height 1080` unless scene bounds are queried directly.
