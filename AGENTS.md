...
- Mutation-path pilot latency conformance now lives in `tze_hud_telemetry::validation` via `evaluate_policy_mutation_latency_conformance` (`policy_mutation_eval_p99`, budget constant `POLICY_MUTATION_EVAL_BUDGET_US = 50`); `session_server` policy-admission logs include this structured conformance payload for CI-visible evidence.
