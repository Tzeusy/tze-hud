...
class TextStreamPortalExemplarTests(unittest.TestCase):
    ...
    def test_input_history_fit_empty_and_zero_budget(self) -> None:
        self.assertEqual(portal.input_history_fit([], 500.0, 500.0), ("", 0.0))
        self.assertEqual(portal.input_history_fit(["x"], 0.0, 500.0), ("", 0.0))

    def test_unified_window_composer_slides_down_as_history_builds(self) -> None:
        saved = list(portal.INPUT_HISTORY_ENTRIES)
        try:
            portal.INPUT_HISTORY_ENTRIES[:] = []
            box = portal._input_pane_content_box()
            empty_composer = portal.input_composer_local_rect()
            # No history: composer owns the full content box.
            self.assertEqual(empty_composer.y, box.y)
            self.assertEqual(empty_composer.h, box.h)
            portal.INPUT_HISTORY_ENTRIES[:] = ["first entry", "second entry"]
            composer = portal.input_composer_local_rect()
            history = portal.input_history_local_rect()
            # History sits at the pane top; composer starts below it.
            self.assertEqual(history.y, box.y)
            self.assertGreater(composer.y, box.y)
            self.assertAlmostEqual(
                composer.y, history.y + history.h + portal.INPUT_HISTORY_GAP,
            )
            self.assertGreater(history.h, 0.0)
        finally:
            portal.INPUT_HISTORY_ENTRIES[:] = saved

    def test_unified_window_composer_never_shrinks_below_min(self) -> None:
        saved = list(portal.INPUT_HISTORY_ENTRIES)
        try:
            portal.INPUT_HISTORY_ENTRIES[:] = [f"entry number {i}" for i in range(500)]
            composer = portal.input_composer_local_rect()
            self.assertGreaterEqual(composer.h, portal.COMPOSER_MIN_H)
        finally:
            portal.INPUT_HISTORY_ENTRIES[:] = saved
