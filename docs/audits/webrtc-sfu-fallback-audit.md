...
| Field | Value |
|---|---|
| Service | Cloudflare Calls (also branded "Cloudflare Realtime") |
...
Cloudflare Calls is a **managed WebRTC SFU service** exposed via REST API and WHIP/WHEP. Like LiveKit, it is **not a Rust transport library** — it is a cloud service. The integration pattern:

```
[tze_hud Rust runtime]
    │ (webrtc-rs or str0m peer-connection)
    │ (WHIP PUT request to Cloudflare Calls API with SDP offer)
    ▼
[Cloudflare Calls — SaaS SFU]
    │ (WebRTC to/from browsers/clients)
    ▼
[Remote participants]
```

### 3.3 WHIP/WHEP Protocol Compatibility

Cloudflare Calls natively supports WHIP (WebRTC-HTTP Ingestion Protocol, RFC draft) for push streams and WHEP (WebRTC-HTTP Egress Protocol) for pull streams. Both are REST-based signaling conventions on top of standard WebRTC:

1. Caller sends HTTP PUT with SDP offer body.
2. Server responds with SDP answer.
3. Standard ICE/DTLS/SRTP negotiation proceeds on the network.

**tze_hud integration**: WHIP is a signaling layer. tze_hud's gRPC signaling plane (RFC 0005/0014) must translate to WHIP for the cloud-relay path. The underlying WebRTC peer-connection (webrtc-rs or str0m) handles the actual ICE/DTLS. This is straightforward to implement with either transport library.

### 3.4 API Maturity

| Aspect | Assessment |
|---|---|
| WHIP support | Generally available (GA) |
| WHEP support | GA |
| SFU features (simulcast forwarding) | Present — Cloudflare Calls handles simulcast forwarding server-side |
| SVC | Not documented; unlikely |
| Recording | Available via Cloudflare Stream integration |
| Data channels | Not supported via Cloudflare Calls |
| API versioning | v1 stable for core push/pull flows |

**Assessment**: Cloudflare Calls is API-mature for the core ingest + fan-out use case. Data channel absence is not a concern (tze_hud uses gRPC for control, not WebRTC data channels).
...
| Proprietary REST API (beyond WHIP/WHEP) | Medium | Room management, track routing, recording are Cloudflare-specific REST APIs — not portable. |
...
```
Phase 4b cloud-relay integration:

1. tze_hud receives embodied session request (gRPC, RFC 0005)
2. Runtime selects cloud-relay path (C15, RFC 0018)
3. tze_hud initiates WHIP session:
   - POST /v1/apps/{app_id}/sessions (Cloudflare Calls API)
   - PUT /v1/apps/{app_id}/sessions/{session_id}/tracks/new  ← SDP offer
   - Receive SDP answer
4. webrtc-rs (or str0m) completes ICE/DTLS using answer
...
The signaling path changes (REST instead of direct SDP exchange); the transport library and GStreamer bridge are unchanged.
...
| **Signaling compatibility** | JSEP SDP (direct or via RFC 0014) | LiveKit protocol; WHIP optional | WHIP/WHEP + Cloudflare API |
...
For cloud-relay (C15), the signaling adapter (WHIP/LiveKit protocol) sits between the gRPC plane and the SFU, regardless of which transport library is used. RFC 0018 (Cloud-Relay Trust Boundary spec, F29 gate) must define this adapter before phase 4b implementation begins.
...
| WHIP signaling compatibility | Confirm str0m + WHIP with LiveKit Server produces correct ICE/DTLS negotiation |
