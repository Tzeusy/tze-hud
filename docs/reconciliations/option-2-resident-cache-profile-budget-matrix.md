...
|---|---|---|---|
| Logical agent texture budget | Agent-leased decoded resource cost, including deliberate per-agent charging of shared content | MiB charged to agent identity while referenced | None. Preserve `max_texture_mb`: 2,048 MiB full-display and 512 MiB headless ([profile defaults](../../about/legends-and-lore/rfcs/0006-configuration.md#L921-L966), [headless defaults](../../about/legends-and-lore/rfcs/0006-configuration.md#L1030-L1053)). |
...
`max_texture_mb` is an authorization/fairness budget, not a promise that its full logical quantity can be made physically resident. A publication can therefore be within its agent texture budget and still be rejected or rendered through a lower-residency path when a resident class is full. The existing delta already requires this separation ([configuration delta](../../openspec/changes/profile-runtime-budget-envelope/specs/configuration/spec.md#L21-L37)).
...
| Full-display widget source 192 MiB | Current 64 MiB gRPC aggregate multiplied by three known simultaneously retained source generations | **Medium-high.** Ownership is code-grounded; MCP/gRPC convergence and copy-count instrumentation must confirm the three-generation assumption. |
...
- **512 MiB resource resident** carries forward RFC 0011’s resource-store total as an actual physical class ceiling ([size defaults](../../about/legends-and-lore/rfcs/0011-resource-store.md#L524-L548)). This is intentionally independent of the 2,048 MiB per-agent logical texture authorization.
- **192 MiB widget source resident** budgets three simultaneous 64 MiB source generations: ingress/fallback ownership, scene/renderer ownership, and the renderer’s compiled render-plan string. Current code moves or copies source through the registration queue and renderer, then clones SVG into a render plan ([registration queue](../../crates/tze_hud_scene/src/graph/overlay.rs#L277-L295), [runtime drain](../../crates/tze_hud_runtime/src/widget_runtime_registration.rs#L10-L22), [render-plan clone](../../crates/tze_hud_compositor/src/widget.rs#L2712-L2743)). This is a conservative class allowance, not permission for one payload to evade existing per-entry limits.
...
|---|---|---|
| Resource resident | Decoded image CPU buffers, uploaded image/resource GPU allocations, and other resource-store retained payloads | Owned immutable slice length; owned `Vec`/`String` capacity; GPU requested allocation footprint from format block size × rounded extent × mip levels × layers × samples. `ResourceRecord.decoded_bytes` remains a logical charge only and does not itself consume resident bytes. |
| Widget source resident | gRPC fallback payload, MCP ingress registry payload, scene pending registration payload, renderer source, and compiled render-plan source | Each simultaneously owned heap buffer by capacity. Moving an owner transfers its charge; copying creates a second charge until the old owner releases. Durable disk blobs are excluded until read into a retained runtime owner. |
...
| Font resident | Bundled source bytes, raw agent font store, compositor/widget font-database copies, shaping and swash caches, and glyph-atlas GPU allocations | Static embedded source length once; owned heap buffer capacity; explicitly wrapped cache allocation size; GPU requested allocation footprint. Shared backing bytes charge once. |

Allocator metadata, hash-table buckets not attributable to a retained payload, executable mappings, driver-private memory, and transient current-frame scratch are excluded, so `accounted_bytes` is not RSS. Transient scratch must remain bounded by existing entry dimensions and frame-working-set guards; this exclusion must not become an unbounded retention escape hatch.
...
5. **Logical authorization remains independent.** Agent logical texture charge is checked as today. Resident admission is a second gate, never a substitution for per-agent authorization.
...
- current and configured logical agent texture charges under a separate `logical_texture_budget` section; and
...
- Add all five fields as optional profile configuration. Omitted fields resolve to the exact built-in matrix above; explicit custom values remain explicit and must pass `sum(class ceilings) <= aggregate` validation.
