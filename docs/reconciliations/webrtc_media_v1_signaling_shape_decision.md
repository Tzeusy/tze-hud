...
For the first bounded post-v1 media ingress slice, signaling SHALL use a **Session delta** shape:

- Extend the existing `HudSession.Session` bidirectional stream envelope.
- Allocate post-v1 media signaling payloads inside the reserved session field range (`50-99`) during `WM-S2b`.
