...
required SSH identities (`hud-user` and `admin-user`) fail before authentication because
...
| TCP 50051 | timed out |
| TCP 9090 | timed out |
| SSH `hud-user` | timed out connecting to port 22 |
| SSH `admin-user` | timed out connecting to port 22 |
...
3. Confirm Windows OpenSSH is running and accepts the existing `~/.ssh/hud-ssh-key`
   key for both `hud-user` and `admin-user`.
