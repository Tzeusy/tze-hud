...
- **[major] F1** — Route composer-interaction enablement onto a stable coalescible surface/overlay path so interaction-enabled streaming batches stay on the StateStream coalescible path, mirroring the accent/unread mitigation (hud-mzk74). **Note:** a naive "emit the `AddNode` once at surface declaration" fix does *not* work — `render_batch` mints a fresh root per `PublishToTile` (`resident_grpc.rs:1099`) and `set_tile_root_checked` removes the old root subtree and prunes dangling part refs (`tiles.rs:416-440`), so a one-shot child `AddNode` is destroyed on the next render and `accepts_composer_input` would vanish after the second render. The fix must express composer-interaction as coalescible surface/overlay state (or reattach the hit region after each root replacement *without* forcing Transactional classification). Add the missing interaction-path regression test alongside the hud-mzk74 non-interactive ones.
