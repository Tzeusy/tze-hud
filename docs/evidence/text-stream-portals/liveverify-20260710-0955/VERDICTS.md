...
Host: tzehouse-windows (`tzehouse-windows.parrot-hen.ts.net`), overlay exe-direct
as `tzeus`, scene 3840×2160, portal 1720×1360 @ (2092,120). MCP 9090 / gRPC 50051.
...
Windows OS pointer/wheel injection (interactive `tzeus` session, scheduled-task
...
| 10 | hud-o9ybl | VERIFIED-FIXED | `composer-edit.log` empty→typed→mid-delete→cleared with caret_x tracked through every state (0→45.6→118.56→109.44→45.6→36.48→0), no phantom chars; `o9ybl-composer-edit-typed.png` shows the draft "Hello" rendered inside the composer box. |
